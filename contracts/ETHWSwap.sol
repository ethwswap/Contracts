...
contract ETHWSwap is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @notice Swap exact input tokens for output tokens in UniswapV3
    /// @param tokenIn the address of token to swap
    /// @param tokenOut the address of token to swap for
    /// @param amountIn tokenIn amount
    /// @param expiration when the transaction expires
    function swapExactTokenIn(
        ...
        if (outPutAmount > 0 && tokenOut == WETH) {
            // Find fortunate in V3 pool with different fee tiers
    ...
    /// @notice Check uniswapV3 pools of token pair A, B with different fee tiers, get the pool with the most liquidity
    /// @return deepestPoolFee the fee rate of the pool
    function getDeepestPoolOfTokenPair(address tokenA, address tokenB) public view returns(uint24 deepestPoolFee) {
        IUniswapV3Factory factory = IUniswapV3Factory(v3Router.factory());

        uint256 maxLiquidity;
        // Get all pools with different fee rates and find the one having the most liquidity
        for (uint256 i = 0; i < tradingFeesV3.length; ++i) {

...
/// @title Flash swap arbitrage implementation
contract UniswapFlashSwap is Ownable, IUniswapV3SwapCallback, ReentrancyGuard {
    ...
    /// @dev Sell WETH for quote tokens in the initial pool, interact with the pool swap() directly
    /// In the swap fallback, we will get quote tokens in THIS address, then we sell quote tokens for WETH in
    /// another pool with different fee tier through uniswap router, expecting to get more WETH than we should pay
    ...
    function initFlashSwap(
        ...
        // Swap quote tokens for WETH
        getPool(_tokenIn, _tokenOut, _fee1).swap(
            address(this),
            zeroForOne,
            // Negative means it's an exact output swap
            -_amount.toInt256(),
            (
            zeroForOne
    ...
    /// @dev Excute arbitrage and pay WETH owned to the Uniswap pool, allocate profits if we gots
    function uniswapV3SwapCallback(
        ...
        // Ensure sender is the pool contract
        ...
        // Withdraw ETHW from WETH contract
